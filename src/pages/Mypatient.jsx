...
const handleCreate = async (e) => {
  e.preventDefault();
  setError("");
  const patient = await patientStore.createPatient(formData);

  if (patient?.success) {
    setIsAddModalOpen(false);
    setRefreshTrigger(prev => prev + 1);
      setShowSuccess(true)
    setMessage(`Success! Patient ${formData.full_name} created successfully`)
    setTimeout(() => {
      setShowSuccess(false)
    }, 2000);
    }
...
const myPatients = patientStore.patients.filter(
  p => p.hospital_id === localStorage.getItem('hospital_id')
);
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 w-fit">Patients</h1>
          <p class="text-gray-600">Manage and add Patients</p></div> 
                <button 
      ...
      stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-plus mr-2 h-4 w-4">
        <path d="M5 12h14"></path><path d="M12 5v14"></path></svg>Add Patients</button>           
            </div>
      ...
      <input className="flex bg-white h-10 w-full rounded-md border border-gray-200 border-input bg-background px-3 py-2 text-base ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm pl-10" placeholder="Search patients by name and by verification status" value={searchQuery}
                  ...
                  <div className="flex flex-col space-y-1.5 p-6">
                    ...
                    <div className="flex-1">
                      <div className="capitalize font-semibold tracking-tight text-lg flex items-center">
                        ...
                        className="lucide lucide-patient mr-2 h-4 w-4"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path>
